...
## Solicitando mais memória do que o chunk superior
No último exemplo, havia um valor de `0x20d11` no chunk superior. O que acontece se usarmos tudo?
...
Antes do uso de arenas, a forma de impedir que mais de uma thread usasse a mesmo heap era através do uso de um mutex. Atualmente, cada arena ainda utiliza um mutex, mas agora as threads podem realizar operações na heap sem se preocuparem umas com as outras (já que estão interagindo com outras arenas). Quando não há como criar mais arenas, algumas threads terão que compartilhar a mesma arena, e o desempenho será mais lento devido ao uso de mutexes para bloquear threads acessando a mesmo heap.
